import { Metadata } from "next";

export const metadata: Metadata = {
	title: {
		default: "Community | Aggie Sports Analytics at UC Davis",
		template: "%s | Aggie Sports Analytics",
	},
	description: "Revolutioning sports success through data science and machine learning at UC Davis.",
	openGraph: {
		title: "Aggie Sports Analytics",
		description:
			"Revolutioning sports success through data science and machine learning.",
		url: "aggiesportsanalytics.com",
		siteName: "aggiesportsanalytics.com",
		images: [
			{
				url: "/centerWhiteLogo.png",
				width: 1920,
				height: 1920,
			},
		],
		locale: "en-US",
		type: "website",
	},
	robots: {
		index: true,
		follow: true,
		googleBot: {
			index: true,
			follow: true,
			"max-video-preview": -1,
			"max-image-preview": "large",
			"max-snippet": -1,
		},
	},
	twitter: {
		title: "AggieSportsAnalytics",
		card: "summary_large_image",
	},
	icons: {
		shortcut: "/favicon.png",
	},
};


export default function CommunityLayout({
	children,
}: { children: React.ReactNode }) {
	return (
		<div className="relative min-h-screen bg-gradient-to-tl  from-zinc-950 from-20% via-purple-950 via-30% to-zinc-950 to-50%">
			{children}
		</div>
	);
}
